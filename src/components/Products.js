...
const Products = () => {
  ...
  useEffect(() => {
    const getProducts = async () => {
      setLoading(true);
      const response = await fetch("http://35.246.127.243:8080/product");
      if (componentMounted) {
  ...
  const filterProduct = (cat) => {
    const updatedList = data.filter((item) => item?.productType === cat);
              ...
              <div className="card text-center h-100" key={product?.productId}>
                ...
                <div className="card-body">
                  ...
                  <p className="card-text">
                    {product?.productDescription?.substring(0, 90)}...
                  </p>
                </div>
                <ul className="list-group list-group-flush">
                  <li className="list-group-item lead">$ {product?.productPrice}</li>
                </ul>
                <div className="card-body">
